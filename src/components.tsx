...
import type { BasketItem, BasketVariant, CheckoutResult, LentaStore, UserProfile, WorkflowStage } from "./types/domain";
...
import { getVariantPresentation, recommendedStrategy } from "./services/variantPresentation";
import { summarizeIntentSlots } from "./services/requestCopy";
import { HomeHeader } from "./components/home/HomeHeader";
import { PixelBasketMark } from "./components/brand/PixelBasketMark";
import { BasketVariantCard } from "./components/results/BasketVariantCard";
import { ResultsHeader } from "./components/results/ResultsHeader";
import { RetailerSelector } from "./components/results/RetailerSelector";
import { FollowUpComposer } from "./components/results/FollowUpComposer";
import { buildRetailerOptions } from "./services/retailerPresentation";
...
export function AppShell({ children, route, authProfile, onOpenOnboarding, onOpenDelivery }: { children: ReactNode; route: "home" | "results"; authProfile: AuthProfile; onOpenOnboarding: () => void; onOpenDelivery: () => void }) {
  ...
  return (
    <main className={`app-shell kit-shell ${route}-route`}>
      {route === "home" && (
        <HomeHeader
          profile={authProfile.profile}
          onOpenDelivery={onOpenDelivery}
          onOpenHelp={onOpenOnboarding}
...
const geoStatusCopy = {
  ...
  success: "Адрес определён. Проверьте его перед сохранением.",
...
export function ConversationPanel({ planner, hasDeliveryAddress, onNeedsDelivery, draft, onDraftChange }: { planner: Planner; hasDeliveryAddress: boolean; onNeedsDelivery?: (request: string) => void; draft?: string; onDraftChange?: (request: string) => void }) {
  ...
  return (
    <section className="conversation-panel kit-home" aria-label="Подбор корзины" data-od-id="conversation-panel">
      ...
      <TaskShortcuts onPick={(fragment) => setText((current) => appendBriefFragment(current, fragment))} />
      ...
      <p className="home-independent-note">{BRAND.independentNote}</p>
    </section>
  );
}

function TaskShortcuts({ onPick }: { onPick: (fragment: string) => void }) {
  return (
    <section className="task-shortcuts" aria-label="Быстро добавить">
      <span>Быстро добавить</span>
      <div className="task-shortcuts__list">
        {taskShortcuts.map((shortcut) => (
          <button key={shortcut.label} type="button" onClick={() => onPick(shortcut.fragment)}>
            {shortcut.label}
          </button>
        ))}
      </div>
...
export function BasketResults({ planner, showResultsHint = false, showBasketEditHint = false, onDismissResultsHint, onDismissBasketEditHint, onVariantOpen, onBasketEdit, onCheckoutClick, onStartNewSearch }: { planner: Planner; showResultsHint?: boolean; showBasketEditHint?: boolean; onDismissResultsHint?: () => void; onDismissBasketEditHint?: () => void; onVariantOpen?: (retailer?: string) => void; onBasketEdit?: (retailer?: string) => void; onCheckoutClick?: (retailer?: string) => void; onStartNewSearch?: () => void }) {
  ...
  const retailerOptions = useMemo(
    () => buildRetailerOptions(variants, planner.state.retailerResults),
    [planner.state.retailerResults, variants],
  );
  const [openedId, setOpenedId] = useState<string | null>(planner.state.selectedId);
  const [activeRetailer, setActiveRetailer] = useState<RetailerKey>(() => retailerOptions[0]?.key ?? "demo");
  const [followUp, setFollowUp] = useState("");
  const [followUpPending, setFollowUpPending] = useState(false);
  const followUpRef = useRef<HTMLTextAreaElement>(null);
  const activeOption = retailerOptions.find((option) => option.key === activeRetailer) ?? retailerOptions[0];
  const activeVariants = activeOption?.variants ?? variants;
  const recommendation = planner.state.intent ? recommendedStrategy(planner.state.intent, activeVariants) : null;
  const selected = variants.find((variant) => variant.id === openedId) ?? null;
  const followUpBusy = ["analyzing", "searching", "composing"].includes(planner.state.stage);
  const selectedPeerVariants = selected
    ? variants.filter((variant) => (variant.retailer ?? "demo") === (selected.retailer ?? "demo"))
    : variants;
  ...
  useEffect(() => {
    if (retailerOptions.length > 0 && !retailerOptions.some((option) => option.key === activeRetailer)) {
      setActiveRetailer(retailerOptions[0].key);
    }
  }, [activeRetailer, retailerOptions]);

  useEffect(() => {
    if (!followUpPending) return;
    if (planner.state.stage === "ready" && !planner.state.error) {
      setFollowUp("");
      setFollowUpPending(false);
      return;
    }
    if (planner.state.stage === "error") setFollowUpPending(false);
  }, [followUpPending, planner.state.error, planner.state.stage]);
    ...
    return (
      <section className="results-panel kit-results basket-step" aria-label="Состав выбранной корзины" data-od-id="results-panel">
        <div className="basket-step-header">
          <button className="link-button step-back liquid-glass" type="button" onClick={backToVariants}>
        ...
        <SelectedBasketActions
          ...
          onBackToVariants={backToVariants}
        />
      </section>
    );
  }
  ...
  return (
    <section className="results-panel kit-results" aria-label="Варианты корзины" data-od-id="results-panel">
      {planner.state.intent ? (
        <ResultsHeader
          intent={planner.state.intent}
          onStartNewSearch={onStartNewSearch ?? (() => undefined)}
          onEditRequest={() => {
        ...
        <header className="results-header">
          <button className="results-header__back" type="button" onClick={onStartNewSearch}>
            <ChevronLeft size={17} aria-hidden="true" /> Новый запрос
          </button>
      ...
      <RetailerSelector options={retailerOptions} activeKey={activeOption?.key ?? activeRetailer} onSelect={setActiveRetailer} />
      <div className="variant-list compare-list" data-od-id="variant-grid">
        {activeVariants.map((variant) => (
          <BasketVariantCard
            key={variant.id}
            variant={variant}
            recommended={variant.strategy === recommendation}
  ...
  return (
    <section className="results-panel empty-results-panel" aria-label="Подборка не найдена" data-od-id="empty-results-panel">
      <div className="empty-state liquid-glass">
        <PixelBasketMark size={64} state="empty" decorative={false} />
  ...
  return (
    <div className="basket-row vv-basket-item-row">
      ...
      <div className="basket-row-copy">
        ...
        {item.priceObservedAt && <span>Цена проверена в {formatObservedAt(item.priceObservedAt)}</span>}
        {item.availability === "unavailable" && <span className="basket-row-unavailable">Нет в наличии</span>}
      </div>
...
export function SelectedBasketActions({
  variant,
  variants,
  mode,
  creating,
  ...
  onReplace: (xmlId: string) => void;
  onCreateCart: () => Promise<CheckoutResult | null>;
  ...
  const storeItem = variant.items.find((item) => item.storeName || item.storeAddress);
  const storeDescription = [storeItem?.storeName, storeItem?.storeAddress].filter(Boolean).join(", ");
  const list = useMemo(() => formatBasketList(variant.items), [variant.items]);
  const copy = () => variant.items.length > 0 && void navigator.clipboard.writeText(list);
  ...
  const checkout = async () => {
    ...
    setCartUrl(result.url ?? null);
  };

  const update = (xmlId: string, quantity: number) => onItems(
    variant.items.map((item) => item.xmlId === xmlId ? { ...item, quantity: Math.min(9, Math.max(1, quantity)) } : item),
  );
  ...
  return (
    <>
      <section className="selected-basket vv-selected-basket" data-od-id="selected-basket">
        <div className="section-heading selected-basket-header">
          <div>
            <p className="section-kicker">Ваша корзина</p>
            <h2>{presentation.title}</h2>
            <p className="selected-basket-retailer">{retailerConfig.label} · {retailerConfig.capabilityLabel}</p>
            {storeDescription && <p className="selected-basket-store">{storeDescription}</p>}
            <p>{variant.totalRub.toLocaleString("ru-RU")} ₽ · {variant.uniqueItemsCount} позиций</p>
          </div>
          <button className="secondary-button" type="button" onClick={copy} disabled={variant.items.length === 0}><Copy size={17} /> Скопировать</button>
        </div>

        {variant.items.length === 0 ? (
          <div className="selected-basket-empty">
            <PixelBasketMark size={84} state="empty" decorative={false} />
            <h2>В корзине больше нет товаров</h2>
            <p>Верните последнюю позицию или выберите другой вариант.</p>
            <div className="selected-basket-empty__actions">
              <button className="secondary-button" type="button" onClick={undoRemove} disabled={!removed}>Вернуть последний товар</button>
              {onBackToVariants && <button className="link-button" type="button" onClick={onBackToVariants}>К вариантам</button>}
            </div>
          </div>
        ) : (
          <div className="rows">
        ...
        {removed && variant.items.length > 0 && (
          <button className="secondary-button undo-button" type="button" onClick={undoRemove}>Вернуть {removed.name}</button>
        )}
        ...
        {isManualList && copyStatus !== "idle" && (
          <p className="demo-note" role="status">
            {copyStatus === "copied"
              ? "Список проверен и скопирован. Добавьте товары в приложении магазина вручную."
              : "Список проверен, но не удалось скопировать автоматически. Используйте кнопку «Скопировать»."}
          </p>
        )}
        {mode === "demo" && <p className="demo-note">Это пример корзины: цены и товары нужны для ориентира. Список можно скопировать.</p>}
        <p className="basket-freshness">Цены, наличие и состав могут измениться. Проверьте товары перед оформлением.</p>
      </section>
      <CheckoutBar
...
function CheckoutBar({ totalRub, itemCount, mode, creating, cartUrl, retailer, onCreateCart }: {
  totalRub: number;
  itemCount: number;
  mode: "live" | "demo" | "connecting";
  creating: boolean;
  ...
  onCreateCart: () => Promise<void>;
}) {
  const label = `${totalRub.toLocaleString("ru-RU")} ₽`;
  const retailerConfig = RETAILERS[retailer];
  const isManualList = retailerConfig.capability === "manual-list";
  const disabled = itemCount === 0 || creating;
  ...
  return (
    <div className="checkout-bar vv-checkout-bar liquid-glass">
      ...
      {mode === "demo" ? (
        <button className="primary-button checkout-button" type="button" disabled>{retailerConfig.unavailableLabel}</button>
      ) : cartUrl && !isManualList ? (
        <a className="primary-button checkout-button" href={cartUrl} target="_blank" rel="noopener noreferrer">
          <ExternalLink size={18} /> {retailerConfig.checkoutLabel}
        </a>
      ) : (
        <button
          ...
          type="button"
          disabled={disabled}
          onClick={() => { if (!disabled) void onCreateCart(); }}
        >
          {creating ? <Loader2 className="spin" size={18} /> : isManualList ? <Copy size={18} /> : <ExternalLink size={18} />}
          {creating ? "Проверяем…" : retailerConfig.checkoutLabel}
        </button>
      )}
    </div>
  );
