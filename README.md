...
Независимый экспериментальный сервис, который превращает запрос обычным языком в сравнимые корзины из реальных каталогов магазинов. Сервис не является официальным приложением ВкусВилла, Ленты или Пятёрочки.

## Продуктовый flow

```text
задача
-> адрес, если он ещё не указан
-> сравнение доступных магазинов
-> три стратегии корзины
-> уточнение запроса
-> редактирование состава
-> автокорзина или проверенный список
```

Pixel art используется как постоянная часть бренда в onboarding, loader, empty states, app icon и social preview.
...
Если переменные Пятёрочки не заданы, этот источник пропускается. Остальные настроенные каталоги продолжают работать независимо.
