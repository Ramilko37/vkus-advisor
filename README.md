...
Яндекс Еда Retail: добавлен отключённый по умолчанию alpha-адаптер. Режим `candidates_only` возвращает товары в `candidateProducts` общего `/api/catalog/search`; до доказанной read-only проверки SKU они не попадают в финальные корзины. `validated` пока недоступен. [Результат live spike и команды повторной проверки](docs/spikes/2026-09-05-yandex-eats-retail.md).
